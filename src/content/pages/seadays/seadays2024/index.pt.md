---
title: 'SeaDays 2024 - Seafile and SeaTable Community-Workshop'
description: 'O evento comunitário em 07.10. e 08.10. em Mainz.'
seo:
    title: ''
    description: ''
date: '2024-04-30'
url: '/pt/seadays2024'

sections:
    - name: hero-5
      weight: 1
      title: SeaDays 2024
      text: 07.10. Seafile & 08.10. Workshop Comunitário SeaTable em Mainz
      classes:
          - bg-gray-200
      buttons:
          - label: Inscreva-se gratuitamente nos SeaDays
            link: 'https://cloud.seatable.io/dtable/forms/baca8c8a-322c-41db-9fd8-9fba282abcd8/'
            style: primary

    - name: 'content-4'
      weight: 2
      title: Encontro da comunidade com os desenvolvedores
      subtitle: O evento do ano
      items:
          - headline: O evento comunitário
            text: Converse diretamente com os desenvolvedores e responsáveis pelo Seafile/SeaTable.<br><br>Saiba mais sobre recursos planejados e planos futuros com um roteiro detalhado.<br>Traga seus desejos e ideias para o SeaDays 2024.
            icon: award
          - headline: 2 dias de palestras empolgantes
            text: Na segunda-feira, **07.10.**, o destaque será a solução de sincronização e compartilhamento Seafile.<br><br>Na terça-feira, **08.10.**, o foco será a plataforma no-code SeaTable.
            icon: comment
          - headline: Para administradores e profissionais
            text: Encontre administradores e responsáveis por projetos de sistemas Seafile/SeaTable de todos os portes.<br><br>No SeaDays 2024, troque experiências com usuários e parceiros.
            icon: graduation-cap

    - name: 'content-13'
      weight: 4
      classes:
          - curved
          - bg-gray-200
      title: Informações sobre o evento
      subtitle: 7 e 8 de outubro de 2024
      description: A universidade é facilmente acessível a pé ou por transporte público tanto da estação central de Mainz quanto do centro da cidade.<br><br>Mais informações sobre como chegar podem ser encontradas na [página da Universidade de Mainz](https://startseite.uni-mainz.de/anreise).<br><br>Se vier de carro, você pode estacionar gratuitamente no campus nos dois dias. A Universidade de Mainz permite que visitantes estacionem gratuitamente por até 30 horas por ano. A cancela abre automaticamente. Mais informações em [Estacionamento na Universidade de Mainz](https://www.verwaltung.zentrale-dienste.uni-mainz.de/informationen-zur-parkraumbewirtschaftung/).
      box-position: [right!, left]
      items:
          - headline: Local
            text: Universidade de Mainz<br>Alemanha
          - headline: Endereço
            text: Anselm-Franz-von-Bentzel-Weg 12<br>55128 Mainz
          - headline: Prédio
            text: Prédio principal NatFak
          - headline: Sala
            text: 7º andar, sala 232, Sala do Senado

    - name: 'content-28'
      weight: 5
      title: Palestrantes e agenda
      text: 'Nos SeaDays 2023 você assistirá a apresentações dos seguintes palestrantes.<br>**Idioma das apresentações: inglês**'
      items:
          - headline: Christoph Dyllick-Brenzinger
            text: Diretor executivo da SeaTable GmbH e datamate GmbH & Co. KG
            link: https://www.linkedin.com/in/christoph-dyllick-brenzinger/
            img: christoph_dyllick.jpg
          - headline: Jonathan Xu
            text: Diretor de tecnologia (CTO) da Seafile Ltd.
            img: gruender-jonathan-xue.jpg
          - headline: Daniel Pan
            text: Diretor executivo (CEO) da Seafile Ltd.
            img: gruender-daniel-pan.png
          - headline: Maciej Brzezniak
            text: Poznan Supercomputing and Networking Center (PSNC)
            img: maciej_brzezniak.jpg
            link: https://www.linkedin.com/in/maciejbrzezniak/
          - headline: Stephan Heinrich
            text: Diretor executivo da Content Butler GmbH
            img: stephan_heinrich.png
            link: https://www.linkedin.com/in/stephanheinrich/
          - headline: Ko Smidt
            text: Professor de “Música” e “Pesquisa & Design” no Johannes Fontanus College (Holanda)
            img: ko-smidt.png
          - headline: Matthias van Laar
            text: Especialista em TI no Johannes Fontanus College (Holanda)
            img: matthias-van-laar.jpg
          - headline: Cristian Livadaru
            text: Diretor executivo da LcX.at
            img: cristian_livadaru.jpg
            link: https://www.linkedin.com/in/livadaru/
          - headline: Philip Möller
            text: Arquiteto de soluções em nuvem na A1 Digital
            img: philip_moeller.jpg
            link: https://www.linkedin.com/in/philip-m%C3%B6ller-8b67659a
          - headline: Stephan Lehniger
            text: Diretor executivo da SecUnity GmbH
            img: stephan_lehniger.jpg
            link: https://www.linkedin.com/in/stephanlehniger/
          - headline: Elias Schneuwly
            text: Diretor executivo da Speicherbox GmbH
            img: elias_schneuwly.jpg
            link: https://www.linkedin.com/in/elias-schneuwly-020825a2
          - headline: Eloy Crespo
            text: VP de vendas e desenvolvimento de negócios @ Collabora Productivity
            img: eloy-crespo.jpg
            link: https://www.linkedin.com/in/eloy-crespo/
          - headline: Lisa Kleinman
            text: Chefe de design de produto @ make.com
            img: lisa-kleinman.jpg
            link: https://www.linkedin.com/in/lkleinman/
          - headline: Seu nome pode estar aqui
            text: E você? Gostaria de fazer uma apresentação nesse evento comunitário?
            img: author-unknown.png

    - name: 'content-26'
      weight: 6
      title: Agenda para 07.10.2024
      subtitle: Dia do Seafile
      template: 'bd93e7f9b1b4465b9791'

    - name: 'content-26'
      weight: 7
      title: Agenda para 08.10.2024
      subtitle: Dia do SeaTable
      template: '91050075fc2941b28e71'
---
