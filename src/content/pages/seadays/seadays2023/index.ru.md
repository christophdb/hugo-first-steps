---
title: 'SeaDays 2023 - Seafile and SeaTable Community-Workshop'
description: 'Общественное мероприятие 28.09. и 29.09. в Майнце.'
seo:
    title: ''
    description: ''
date: '2024-04-30'
url: '/ru/seadays2023'

sections:
    - name: hero-5
      weight: 1
      title: SeaDays 2023
      text: 28.09. Seafile и 29.09. Сообщество-воркшоп SeaTable в Майнце
      classes:
          - bg-gray-200
      buttons:
          - label: Бесплатная регистрация на SeaDays
            link: 'https://cloud.seatable.io/dtable/forms/48e601f6-6512-4f95-9c79-56ce92c6e271/'
            style: primary

    - name: 'content-4'
      weight: 2
      title: Встреча сообщества с разработчиками
      subtitle: Событие года
      items:
          - headline: Сообщество-ивент
            text: Поговорите напрямую с разработчиками и ответственными за Seafile/SeaTable.<br><br>Узнайте больше о планируемых функциях и планах развития с подробной дорожной картой.<br>Поделитесь своими пожеланиями и идеями.
            icon: award
          - headline: 2 дня интересных докладов
            text: В четверг, **28.09.**, всё будет посвящено решению для синхронизации и обмена файлами Seafile.<br><br>В пятницу, **29.09.**, в центре внимания окажется no-code платформа SeaTable.
            icon: comment
          - headline: Для администраторов и профессионалов
            text: Встретьтесь с администраторами и руководителями проектов крупных и небольших систем Seafile/SeaTable.<br><br>Обменивайтесь опытом с пользователями и партнёрами.
            icon: graduation-cap

    - name: 'content-1'
      weight: 3
      title: Опросы во время мероприятия
      description: 'Во время мероприятия можно пройти следующие опросы:'
      buttons:
          - label: 'Мероприятие (общее)'
            link: https://cloud.seatable.io/dtable/forms/b811ff13-85e0-4d7c-a9d1-4e0fcc224998/
            style: text
          - label: 'День Seafile (28.09)'
            link: https://cloud.seatable.io/dtable/forms/cca759a0-4835-4511-bf8a-33bb6d2f7cee/
            style: text
          - label: 'День SeaTable (29.09)'
            link: https://cloud.seatable.io/dtable/forms/39125ce6-d5cd-4fa2-a339-dcb569dd70b0/
            style: text

    - name: 'content-13'
      weight: 4
      classes:
          - curved
          - bg-gray-200
      title: Информация о мероприятии
      subtitle: 28 и 29 сентября 2023 года
      description: Университет удобно расположен — можно дойти пешком или доехать на общественном транспорте как от главного вокзала Майнца, так и из центра города.<br><br>Больше информации о маршруте смотрите на [сайте Университета Майнца](https://startseite.uni-mainz.de/anreise).<br><br>Если вы приедете на автомобиле, используйте **парковки на Albert-Schweitzer-Straße**. Если вам нужен пропуск на въезд, оформить его нужно самостоятельно.
      box-position: [right!, left]
      items:
          - headline: Место проведения
            text: Университет Майнца<br>Германия
          - headline: Адрес
            text: Anselm-Franz-von-Bentzel-Weg 12<br>55128 Майнц
          - headline: Здание
            text: Главное здание NatFak
          - headline: Аудитория
            text: 7 этаж, аудитория 232, Зал Сената

    - name: 'content-28'
      weight: 5
      title: Спикеры и программа
      text: 'На SeaDays 2023 вас ждут выступления следующих спикеров.<br>**Язык презентаций: английский**'
      items:
          - headline: Кристоф Диллик-Бренцингер
            text: Генеральный директор SeaTable GmbH и datamate GmbH & Co. KG
            link: https://www.linkedin.com/in/christoph-dyllick-brenzinger/
            img: christoph_dyllick.jpg
          - headline: Джонатан Сюй
            text: Технический директор (CTO) Seafile Ltd.
            img: gruender-jonathan-xue.jpg
          - headline: Даниэль Пан
            text: Генеральный директор (CEO) Seafile Ltd.
            img: gruender-daniel-pan.png
          - headline: Мартин Роте
            text: Офицер по информационной безопасности (ISB) Школы информационных технологий Бундесвера (ITSBw) и руководитель инновационного проекта No-Code-DB Bw (BwTable ITSBw)
            img: martin_rothe.jpg
            link: https://www.linkedin.com/in/martin-rothe-a69052114/
          - headline: Элиас Шнойвли
            text: Генеральный директор Speicherbox GmbH
            img: elias_schneuwly.jpg
            link: https://www.linkedin.com/in/elias-schneuwly-020825a2
          - headline: Мориц Шларб
            text: Системный администратор Linux в Университете имени Иоганна Гутенберга в Майнце
            img: moritz-schlarb.jpg
            link: https://www.linkedin.com/in/moritz-schlarb-848b7b241/
          - headline: Богдан Василе
            text: Облачный архитектор в Обществе Макса Планка
            link: https://www.linkedin.com/in/bogdan-vasile-0a299163/
            img: bogdan_seadays.png
          - headline: Юлиан Гуттцайт
            text: Генеральный директор 5pace | Кино- и медиапродакшн
            img: julian_guttzeit.jpg
            link: https://www.linkedin.com/in/julianguttzeit/
          - headline: Майкл Микс
            text: Генеральный директор Collabora Productivity
            img: michael-meeks.png
            link: https://www.linkedin.com/in/michael-meeks-8a6b2151/
          - headline: Клара Родер
            text: Partner Manager в //SEIBERT/MEDIA GmbH
            img: clara-roder.jpg
            link: https://www.linkedin.com/in/clararoder/
          - headline: Здесь может быть ваше имя
            text: А вы? Хотите выступить с докладом на этом мероприятии сообщества?
            img: author-unknown.png

    - name: 'content-26'
      weight: 6
      title: Программа на 28.09.2023
      subtitle: День Seafile
      template: '9710e23245d44887a892'

    - name: 'content-26'
      weight: 7
      title: Программа на 29.09.2023
      subtitle: День SeaTable
      template: '693ad9bd24a746e58fa3'
---
