...
The full list of changes is, as always, available in [SeaTable's Changelog](/en/docs/changelog/version-1-5/). This release note presents the new version's biggest and most exciting changes.
...
This release note only covers the most important improvements in greater detail. The Changelog contains the complete list of modifications. Check it out! Among the noteworthy smaller improvements are time triggers for Python scripts (see the [release note of SeaTable 1.4](/en/seatable-release-1-4/?lang=auto)) and the removal of performance issues when handling a larger number of rows in the table editor.
