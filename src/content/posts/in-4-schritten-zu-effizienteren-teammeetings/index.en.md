...
The [chat function]({{< relref "help/base-editor/zeilen/zeilen-kommentieren" >}}) in each row, i.e. in each individual team meeting, enables uncomplicated and streamlined communication that makes any e-mails superfluous. When you open row (click on the small blue arrows on the left in row), an overall view of the meeting opens. On the right is the chat, where you can easily add employees and write important messages, such as that employee Müller should please upload the interim presentation with the annual review.
