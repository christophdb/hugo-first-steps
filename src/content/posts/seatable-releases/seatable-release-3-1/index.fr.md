...
Grâce à cette division des données en deux, l'utilisateur gagne une énorme flexibilité supplémentaire. Il est possible de mettre en œuvre des projets collaboratifs dans lesquels les membres de l'équipe travaillent dans l'interface web, tout comme des applications à forte intensité de données avec une saisie automatisée des données. Étant donné que les jeux de données dans le stockage de données volumineuses ne supportent pas la collaboration en ligne, c'est-à-dire que les modifications apportées aux données ne sont pas poussées vers d'autres utilisateurs, mais ne sont visibles qu'après une nouvelle consultation, ce stockage est particulièrement adapté aux jeux de données sur lesquels aucun travail actif n'est effectué. Cela explique également pourquoi la version bêta de la fonction Big Data de SeaTable 2.3 était encore [appelée Archives](/fr/seatable-release-2-3/).
