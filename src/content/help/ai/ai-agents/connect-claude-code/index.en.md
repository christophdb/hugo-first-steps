...
- Claude Code (installable via `npm install -g @anthropic-ai/claude-code`, details at [docs.anthropic.com](https://docs.anthropic.com/en/docs/claude-code/overview))
...
| Claude says it has no access | Check with `claude mcp list` whether the MCP server is correctly registered. |
| MCP server is not recognized | Restart Claude Code and try again. |
| Claude suddenly loses access | Claude Code currently does not automatically reconnect when the session has expired. Type `/mcp` and select **Reconnect** to restore the connection. |
