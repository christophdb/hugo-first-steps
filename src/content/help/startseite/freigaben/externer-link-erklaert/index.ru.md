...
Внешняя ссылка - это **URL-адрес**, который предоставляет вам доступ **только для чтения** к коллекции данных в SeaTable. Такой сбор данных может быть [базой](/ru/docs/handbuch/datenmanagement/bases/) или [представлением](/ru/docs/handbuch/datenmanagement/ansichten/).

- **Внешняя ссылка для базы**: Вызвав эту внешнюю ссылку, посетитель может **увидеть** все данные, хранящиеся в базе, включая все таблицы. Посетитель также может получить доступ ко **всем существующим просмотрам** и [статистике](/ru/docs/handbuch/datenmanagement/statistiken/). Кроме того, он может использовать **инструменты оценки** - [группировку, сортировку и фильтрацию](/ru/docs/handbuch/datenmanagement/gruppierung-sortierung-filter/).
- Внешняя **ссылка для представления**: такая внешняя ссылка предоставляет **доступ на чтение к _определенному_ представлению таблицы**. Другие представления и таблицы в Base, а также **скрытые строки и столбцы** остаются **скрытыми** от пользователей внешней ссылки. Подробнее об этом читайте в статье [Создание внешней ссылки для представления](https://seatable.io/ru/docs/ansichtsfreigaben/externen-link-fuer-eine-ansicht-erstellen/).

Внешние ссылки в основном являются **общедоступными**, т.е. ссылка может быть доступна любому человеку, независимо от того, вошел он в SeaTable или нет. Это также является одним из основных [отличий от пригласительной ссылки](https://seatable.io/ru/docs/freigabelinks/unterschiede-zwischen-einladungs-links-und-externen-links/). **Пригласительная ссылка** требует входа в SeaTable и, следовательно, учетной записи пользователя. Подробнее об этом читайте в статье [Создание пригласительной ссылки](https://seatable.io/ru/docs/freigabelinks/einladungs-link-erklaert/).
...
С помощью простого _тега embed_ вы можете интегрировать базу или представление в веб-страницу через внешнюю ссылку. Например, базы в нашей [библиотеке шаблонов](/ru/docs/templates/) встроены в наш сайт через внешние ссылки. Таким же образом мы публикуем статьи для прессы SeaTable на нашей странице для прессы.
