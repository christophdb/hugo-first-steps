...
Внешняя ссылка - это **URL-адрес**, который предоставляет вам доступ **только для чтения** к коллекции данных в SeaTable. Такой сбор данных может быть [базой]({{< relref "startseite/bases/allgemeines-zu-bases" >}}) или [представлением]({{< relref "base-editor/einfuehrung/ansichten/" >}}).

- **Внешняя ссылка для базы**: Вызвав эту внешнюю ссылку, посетитель может **увидеть** все данные, хранящиеся в базе, включая все таблицы. Посетитель также может получить доступ ко **всем существующим просмотрам** и [статистике]({{< relref "base-editor/statistiken/anleitung-zum-statistik-modul" >}}). Кроме того, он может использовать **инструменты оценки** - [группировку, сортировку и фильтрацию]({{< relref "base-editor/ansichten/gruppierung-sortierung-und-filter" >}}).
...
С помощью простого _тега embed_ вы можете интегрировать базу или представление в веб-страницу через внешнюю ссылку. Например, базы в нашей [библиотеке шаблонов](https://seatable.io/vorlagen/) встроены в наш сайт через внешние ссылки. Таким же образом мы публикуем статьи для прессы SeaTable на нашей странице для прессы.
