---
title: 'Asignar permisos en un grupo'
date: 2022-09-26
lastmod: '2023-07-20'
categories:
    - 'arbeiten-mit-gruppen'
author: 'nsc2'
url: '/es/ayuda/permisos-grupo-asignar-seatable'
aliases:
    - '/es/ayuda/berechtigungen-in-einer-gruppe-vergeben'
seo:
    title: 'Permisos de grupo en SeaTable: asigna los roles clave'
    description: 'Gestiona roles de propietario, administrador o miembro y controla los permisos de acceso en SeaTable para cada grupo de trabajo, de forma clara y segura.'

---

Dentro de un grupo SeaTable, puede asignar permisos asignando un rol específico a los miembros del grupo. Hay que distinguir entre los roles de **propietario**, **administrador** y **miembro**, que tienen [permisos]({{< relref "help/startseite/gruppen/gruppenmitglieder-und-ihre-berechtigungen" >}}) diferentes.

Es importante tener en cuenta que sólo como **propietario** o **administrador de** un grupo puedes cambiar los permisos de los demás miembros. Aquí puedes ver cómo asignar permisos en un grupo:

## Asignación de permisos en un grupo

1. Cambie a la página de **inicio** de SeaTable.
2. Haga clic en el **símbolo del triángulo** situado a la derecha del nombre del grupo al que desea asignar permisos.
3. Vaya a **Gestionar miembros**.

![Gestionar los miembros](images/manage-members.jpg)

5. Para editar, haga clic en el **icono del lápiz situado** a la derecha de la función actual del miembro del grupo correspondiente.

![Haga clic en el icono del lápiz situado junto a cualquier miembro del grupo.](images/click-edit-symbol.jpg)

7. Seleccione la **función** que desea asignar al usuario correspondiente haciendo clic en el cuadro de selección.
8. Confirme su selección haciendo clic en el campo **Cerrar**.

![Selección y confirmación de funciones](images/change-authorization-and-close.jpg)

{{< warning  headline="Nota importante" >}}

Si ya no está seguro de cuál de los tres roles tiene qué permisos, esta [tabla sobre los permisos de los roles]({{< relref "help/startseite/gruppen/gruppenmitglieder-und-ihre-berechtigungen" >}}) le ayudará.

{{< /warning >}}
