...
In addition, you can select as a temporal trigger that the **deadline of an entry is approaching**. This trigger is only applicable if there is a [date column]({{< relref "help/base-editor/spalten/die-datum-spalte" >}}die-datum-spalte/) in the selected table view. You can then specify from **when** and **how often** SeaTable sends **reminders**.
