...
{{< seatable-icon icon="dtable-icon-script" >}} Na nova Base, abra a [barra lateral de scripts](https://seatable.io/pt/docs/javascript-python/anlegen-und-loeschen-eines-skriptes/) clicando no ícone de script no cabeçalho da Base, no canto superior direito. Em seguida, selecione **Adicionar script**. O script de migração é escrito na linguagem de programação Python. Portanto, selecione **Python**.
...
Para que o SeaTable importe os dados corretamente, é necessário mais um passo: a especificação das [colunas de ligação](https://seatable.io/pt/docs/verknuepfungen/wie-man-tabellen-in-seatable-miteinander-verknuepft/) na Airtable Base.
...
Se não estiver satisfeito com o resultado final, [apague todas as tabelas](https://seatable.io/pt/docs/arbeiten-in-tabellen/loeschen-einer-tabelle-aus-einer-base/) e recomece no passo 5.
