...
- Для типов столбцов с визуальным выделением – например, столбцов [Единственный выбор]({{< relref "help/base-editor/spaltentypen/die-einfachauswahl-spalte" >}}) или [Сотрудник]({{< relref "help/base-editor/spaltentypen/die-mitarbeiter-spalte" >}}) – **метки** при желании могут отображаться как обычный **текст**.
- Если Вы нажмете на адрес электронной почты в колонке [e-mail]({{< relref "help/base-editor/spaltentypen/die-e-mail-spalte" >}}), откроется почтовый клиент, установленный на Вашем устройстве (например, Microsoft Outlook или Mozilla Thunderbird).
- Если Вы нажмете на ссылку в колонке [URL]({{< relref "help/base-editor/spaltentypen/die-url-spalte" >}}), соответствующий веб-сайт откроется в новой вкладке Вашего браузера.
- Если Вы нажмете на символ миниатюры в колонке [Файл]({{< relref "help/base-editor/spaltentypen/die-datei-spalte" >}}), файл откроется в подходящем средстве просмотра в новой вкладке Вашего браузера. Если настроен редактор Online Office Editor, Вы также можете открывать и редактировать документы Office прямо в браузере.
- С помощью [Связывающих столбцов]({{< relref "help/base-editor/spaltentypen/die-verknuepfungsspalte" >}}) все записи, связанные с записью данных, могут быть четко отображены в таблице. Вы можете настроить ширину столбцов, а также скрыть столбцы с помощью настроек элемента, чтобы отображалась только основная информация о связанных записях.
...
При наличии соответствующих полномочий пользователи приложения могут редактировать поля таблицы отдельных записей данных на этой странице.
...
Для еще более плавной и интуитивно понятной работы отдельные поля таблицы можно редактировать **прямо на странице**. Для этого нажмите на поле и внесите изменения в **режиме редактирования**. Они сохраняются сразу же.

![Встроенное редактирование на страницах с одной записью](images/inline-editing-single-record-page.png)
...
В **настройках столбца связи** можно указать для каждой связанной таблицы, какие данные будут видимыми и редактируемыми и какие операции разрешены. В зависимости от конфигурации пользователи могут создавать новые записи в связанной таблице, а также связывать, изменять и удалять существующие записи. Это обеспечивает точное управление соответствующими **полномочиями**.
...
- **Настройки столбцов связанной таблицы**: Здесь Вы можете определить, в какой мере следующие полномочия применяются к определенным столбцам.
- **Создавать и связывать новые записи**: если вы активируете этот ползунок, пользователи смогут добавлять новые записи в связанную таблицу. Вы можете использовать настройки столбцов, чтобы определить, какие столбцы будут **видимыми**, а какие - **обязательными**, т. е. подлежащими заполнению.
- **Связывать существующие записи**: если вы активируете этот ползунок, пользователи смогут связывать существующие записи в связанной таблице. Вы можете использовать настройки столбцов, чтобы определить, какие столбцы будут **видны**.
...
- **Редактировать связанные записи**: Если Вы активируете этот ползунок, пользователи смогут редактировать существующие записи в связанной таблице. Вы можете использовать настройки столбцов, чтобы определить, какие столбцы будут **редактируемыми**.
- **Удалять существующие записи**: С помощью этого полномочия можно точно определить, разрешено ли пользователям удалять существующие связи.
