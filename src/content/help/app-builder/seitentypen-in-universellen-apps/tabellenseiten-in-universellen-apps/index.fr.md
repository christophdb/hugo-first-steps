...
Vous pouvez activer un autre **curseur** pour [valider les entrées dans les colonnes de texte](https://seatable.io/fr/docs/text-und-zahlen/validierung-der-eingabe-bei-textfeldern/). Dès que la **vérification du format** est activée, les détails de la ligne s'ouvrent toujours lorsque quelqu'un veut ajouter une nouvelle entrée sur la page du tableau - et cela n'est possible que si les entrées correspondent au format défini. Pour les entrées dont le format est différent, un **message d'erreur** apparaît.

![Vérification du format sur la page du tableau](images/Format-check-on-table-page.png)

Vous pouvez par exemple minimiser **les erreurs de frappe** sur des chaînes de caractères clairement définies, comme les codes postaux ou les numéros de compte. Vous définissez les [expressions régulières](https://seatable.io/fr/docs/text-und-zahlen/validierung-der-eingabe-bei-textfeldern/) pour valider les entrées dès la création d'une colonne de texte dans la base.
...
Dans les [autorisations de page](https://seatable.io/fr/docs/apps/seitenberechtigungen-in-einer-universellen-app/), vous pouvez définir avec précision qui peut voir et modifier le tableau.
