---
title: 'Импорт и экспорт сценария'
date: 2023-03-10
lastmod: '2023-03-10'
categories:
    - 'javascript-python'
author: 'nsc2'
url: '/ru/pomoshch/import-export-skriptov-seatable'
aliases:
    - '/ru/pomoshch/import-und-export-eines-skriptes'
seo:
    title: 'Импорт и экспорт скриптов в базе SeaTable'
    description: 'Импортируйте или скачайте скрипты (JS/Python) в базе SeaTable: удобная инструкция по загрузке и выгрузке файлов для работы и резервного копирования.'

---

SeaTable позволяет вам выполнять отдельные сценарии JavaScript и Python в базе. Помимо возможности писать сценарии вручную в текстовом поле, у вас есть возможность импортировать сценарии в SeaTable, а также экспортировать существующие сценарии.

## Импорт сценария

![Импорт сценария](images/Anlegen-eines-Skriptes.jpg)

1. В своей базе нажмите {{< seatable-icon icon="dtable-icon-script" >}} в заголовке базы.
2. Нажмите кнопку **Импорт**.

![Импорт сценария в вашу базу](images/import-scripts.jpg)

4. Выберите **файл сценария .js или .py** с вашего устройства для импорта.
5. Импортированный **сценарий** автоматически сохраняется в SeaTable и добавляется к существующим сценариям.

## Экспорт сценария

![Экспорт сценария](images/Anlegen-eines-Skriptes.jpg)

1. В своей базе нажмите {{< seatable-icon icon="dtable-icon-script" >}} в заголовке базы.
2. Наведите курсор мыши на **название** вашего сценария.
3. Нажмите на **три точки** {{< seatable-icon icon="dtable-icon-more-level" >}}.
4. Нажмите на кнопку **Экспорт**.
5. Затем выбранный **сценарий** загружается и сохраняется в виде файла на вашем устройстве.

![Экспорт сценария ](images/export-scripts.png)
