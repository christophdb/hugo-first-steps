---
title: 'Дублирование таблицы'
date: 2024-02-20
lastmod: '2024-02-20'
categories:
    - 'arbeiten-in-tabellen'
author: 'kgr'
url: '/ru/pomoshch/dublirovanie-tablitsy'
aliases:
    - '/ru/pomoshch/duplizieren-einer-tabelle'
seo:
    title: 'Дублирование таблицы: копия структуры и данных'
    description: 'Копируйте таблицы в SeaTable: полностью или только структуру, с настройками и записями – удобно для бюджета, отчетов и шаблонов.'

---

Вам нужно, чтобы таблица в вашей базе несколько раз использовалась в несколько ином виде? Тогда вы можете продублировать ее всего несколькими щелчками мыши. Это так просто:

1. В заголовке базы вы найдете все таблицы базы, отображаемые в виде **вкладок** в левом верхнем углу.
2. Нажмите на **раскрывающийся значок** рядом с **названием таблицы**, которую нужно продублировать.

![Дублирование структуры таблицы](images/Tabellenstruktur-duplizieren.png)

4. Нажмите на кнопку **Дублировать таблицу**.
5. Чтобы решить, дублировать ли таблицу **пустой или с записями**, активируйте ползунок.

![Диалог: Дублирование записей при дублировании таблицы](images/Dialog-Eintraege-duplizieren-wenn-Tabelle-dupliziert-wird.png)

7. Подтвердите с помощью кнопки **Отправить**.

{{< warning  headline="Примечание" >}}

Все представления также дублируются со всеми [настройками представлений]({{< relref "help/base-editor/ansichten" >}}), так что скопированная таблица является точной копией исходной таблицы.

{{< /warning >}}

**Вот как выглядит результат с дублированными записями**

![Дублированная таблица с дублирующимися записями](images/Duplizierte-Tabelle-mit-duplizierten-Eintraegen.png)

Эта опция полезна, если вы хотите скопировать хотя бы часть строк из исходной таблицы.

**Вот как выглядит результат без дублированных записей**

![Удалите дублирующиеся таблицы](images/Leere-duplizierte-Tabellen.png)

Этот вариант удобен, если вы хотите сохранить только **структуру** исходной таблицы, но заполнить ее другими записями. Это может произойти, например, если вы создаете одну таблицу на год для [планирования бюджета]({{< relref "templates/finance/budget-plan" >}}).
