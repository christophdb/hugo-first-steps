---
title: 'Восстановление удаленной таблицы'
date: 2022-11-17
lastmod: '2023-01-12'
categories:
    - 'arbeiten-in-tabellen'
author: 'nsc2'
url: '/ru/pomoshch/vosstanovit-udalennuyu-tablitsu-seatable'
aliases:
    - '/ru/pomoshch/eine-geloeschte-tabelle-wiederherstellen'
seo:
    title: 'Восстановить удалённую таблицу в SeaTable: пошагово'
    description: 'В SeaTable можно быстро вернуть удалённую таблицу, строку или столбец через корзину — подробная инструкция по восстановлению данных.'

---

В SeaTable вам не нужно беспокоиться о потере данных. Даже случайно удаленные таблицы - и даже отдельные столбцы и строки - могут быть легко восстановлены после этого через **корзину** в **базовых опциях**.

## Восстановление удаленной таблицы

![Восстановление таблиц](images/Wiederherstellung-von-Tabellen.png)

2. Нажмите на **Версии** в правом верхнем углу опций базы.
3. Откройте **корзину**.
4. Теперь вы увидите все **таблицы**, **строки** и **столбцы**, которые были удалены в этой базе в прошлом.
5. Выберите нужную **таблицу**, которую вы хотите восстановить, и нажмите кнопку **Восстановить** справа от времени удаления.
6. Затем выбранный стол автоматически добавляется обратно в вашу **базу**.

## Восстанавливаемое содержимое таблицы

Когда вы извлекаете таблицу из корзины, она восстанавливается в том виде, в котором она выглядела при удалении. Сюда входят следующие компоненты:

- Данные таблицы
- [Виды таблиц]({{< relref "help/base-editor/ansichten" >}})
- [Комментарии]({{< relref "help/base-editor/zeilen/zeilen-kommentieren" >}})
- [Веб-формы]({{< relref "help/base-editor/webformulare" >}})
- Автоматизация
- Представления и оценки в плагинах SeaTable

{{< warning headline="Ссылки на удаленную таблицу должны быть восстановлены отдельно" text="Одной из сильных сторон SeaTable является возможность связывать таблицы между собой. Если вы удалите таблицу, все связывающие столбцы, для которых эта удаленная таблица была целевой, исчезнут. Если теперь восстановить исходную таблицу, связывающие столбцы не будут автоматически воссозданы, но их также можно восстановить из корзины. Таким образом, информация не теряется." />}}

![Восстановление удаленного столбца ссылок](images/restore-deleted-link-column.png)

## Больше полезных статей о восстановлении данных

SeaTable предлагает различные варианты восстановления удаленного содержимого. Можно восстановить не только удаленные таблицы, но и последние изменения, старые версии или целые базы. [Обзорная статья о восстановлении данных в SeaTable]({{< relref "help/base-editor/historie-und-versionen/moeglichkeiten-der-datenwiederherstellung" >}}) обязательно поможет вам.
