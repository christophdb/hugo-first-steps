...
A concrete use case for this data processing operation could occur, for example, if you want to collect the number of daily accesses to a website in your table. For this purpose, you want to **add up** the **access numbers of** each individual day, which are recorded in an existing [number]({{< relref "help/base-editor/numerische-spalten/die-zahlen-spalte" >}}) column, from row to row in a new column.
