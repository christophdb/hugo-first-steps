...
С помощью идентификатора пользователя можно устанавливать **динамические фильтры** в SeaTable, которые в определенных ситуациях подходят лучше, чем фильтр по [колонке создателя]({{< relref "help/base-editor/autofill-spalten/die-spalten-ersteller-und-erstelldatum" >}}). В этой статье мы представляем преимущества и недостатки фильтров с идентификатором пользователя.
...
Альтернативой может быть фильтрация по столбцу " [Создатель"]({{< relref "help/base-editor/auswahl-spalten/die-spalte-mitarbeiter" >}}), которые также предлагают динамические возможности фильтрации. Однако эти два столбца имеют следующие недостатки:
