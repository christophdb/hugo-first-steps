---
title: 'Eliminar una vista - SeaTable'
date: 2022-10-26
lastmod: '2023-01-02'
categories:
    - 'grundlagen-von-ansichten'
author: 'vge'
url: '/es/ayuda/loeschen-einer-ansicht'
---

Puede eliminar las vistas que se hayan creado incorrectamente o que hayan dejado de ser pertinentes en cualquier momento haciendo clic en el menú de vistas y seleccionando la opción **Eliminar** vista.

## Borrar una vista

1. Haga clic en el **nombre de la vista actual**.
2. Mueva el ratón sobre la vista que desea eliminar.
3. **Tres puntos** se hacen visibles en la línea de visión. Selecciónalos.
4. Seleccione la opción **Borrar vista**.

![Borrar una vista](images/Loeschen-einer-Ansicht-1.gif)

## Preguntas frecuentes

{{< faq >}}

¿Se puede recuperar una vista borrada?|||

**No**. Dado que una vista no contiene datos propios, sino que es simplemente una representación especial de los datos, una vista eliminada no puede borrarse ni a través de la papelera de reciclaje ni a través del acceso directo {{< keyboard "CTRL" >}} + {{< keyboard "Z" >}} Restaurar.

---

¿Se puede eliminar siempre una vista?|||

Sólo puede eliminar vistas si ha creado **al menos una vista adicional** a la vista por defecto.

{{< /faq >}}
