...
В другой таблице ("Журнал состояния заказов") также необходимо видеть различные **номера заказов** вместе с соответствующими **суммами**. Кроме того, в этой таблице в [столбце]({{< relref "help/base-editor/numerische-spalten/die-datum-spalte" >}}) даты "Последнее изменение статуса" указывается **время последнего** изменения статуса.
