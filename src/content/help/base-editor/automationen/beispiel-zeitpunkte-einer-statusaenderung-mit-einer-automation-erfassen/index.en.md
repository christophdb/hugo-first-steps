...
In another table ("Order status log") you also want to see the different **order numbers** together with the corresponding **amounts**. In addition, the [date column](https://seatable.io/en/docs/datum-dauer-und-personen/die-datum-spalte/) "last status change" in this table contains the **time of the last status change**.
