...
In another table ("Order status log") you also want to see the different **order numbers** together with the corresponding **amounts**. In addition, the [date column]({{< relref "help/base-editor/spalten/die-datum-spalte" >}}die-datum-spalte/) "last status change" in this table contains the **time of the last status change**.
