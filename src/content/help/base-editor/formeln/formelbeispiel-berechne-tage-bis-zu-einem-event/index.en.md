...
categories:
    - 'formeln'
author: 'nsc2'
url: '/help/formula-calculate-days-until-event'
aliases:
    - '/help/formelbeispiel-berechne-tage-bis-zu-einem-event'
seo:
    title: 'Formula to calculate days until an event in SeaTable'
    description: 'Use a SeaTable formula to measure remaining days until an event or deadline – great for scheduling and deadline tracking.'
...
![Example table Calculate days until an event with a formula](images/example-table-days-until-event.png)

To do this, first add a **formula column** to the table, in whose editor you can then insert the **formula**.

![Adding a formula in the text field provided for this purpose](images/insert-formular-example-days-until-event.png)
...
![Adding the function "now" and closing the function with a bracket](images/add-now-and-close-the-.png)
...
![Adding the reference to the name of the table column where the event data can be found.](images/add-reference-to-event-date.png)
...
![Exit the formula with the calculation methodology ](images/finish-the-formular-example-days-until-event.png)
...
![After confirming the formula, the remaining days will be automatically calculated and added to the formula column of your table.](images/table-after-formular-days-until-events.png)
