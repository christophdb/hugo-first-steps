...
Формулы SeaTable предлагают различные способы работы с данными в таблицах. Одним из них является поиск **слов в тексте**. Введя соответствующую формулу, можно автоматически искать в [текстовом столбце]({{< relref "help/base-editor/text-spalten/die-spalten-text-und-formatierter-text" >}}) какие-либо слова и получать подсказку - например, "слово найдено/не найдено" - в виде результата в столбце формул таблицы.
