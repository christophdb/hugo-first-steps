...
With the help of User ID you can set **dynamic filters** in SeaTable, which in certain situations is more suitable than filtering by the [creator column](https://seatable.io/en/docs/datum-dauer-und-personen/die-spalten-ersteller-und-erstellt/). In this article we will present you the advantages and disadvantages of filters with User ID.
...
In SeaTable you can use the filter **"is current user's ID"** for each text column. This creates a **dynamic filter** that only shows entries where the **own user** 's ID is stored in the corresponding [text column](https://seatable.io/en/docs/text-und-zahlen/die-spalten-text-und-formatierter-text/).
