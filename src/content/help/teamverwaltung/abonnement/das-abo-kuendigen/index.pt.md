---
title: 'Cancelar a assinatura'
date: 2023-01-28
lastmod: '2023-01-28'
categories:
    - 'abo-abrechnung'
author: 'nsc2'
url: '/pt/ajuda/das-abo-kuendigen'
---

Gostaria de cancelar a sua subscrição SeaTable? Para o fazer, utilizar o separador Subscrição na administração da equipa. Note-se, contudo, que o cancelamento das subscrições existentes só tem efeito no final do prazo. Pode ainda fazer pleno uso da sua subscrição cancelada até este ponto ser alcançado.

## Cancelar subscrição

{{< warning  headline="Nota importante"  text="O pacote de assinatura da sua equipa só pode ser cancelado pelos **administradores da equipa**." />}}2. Abrir a **administração da equipa**. 3. Clique em **Subscrição**. 4. Clique em **"Cancelar a subscrição e mudar para o SeaTable Free"**. 5. A sua subscrição foi agora cancelada no **final** do **período** que seleccionou. Ainda se pode fazer pleno uso do SeaTable até esta data.

![As subscrições existentes podem ser canceladas através da administração da equipa](images/Abonnement-loeschen.jpg)

## Eliminar completamente a equipa

Se quiser não só cancelar a sua subscrição actual, mas também apagar toda a sua equipa, leia o artigo de ajuda [Apagar toda a equipa](https://seatable.io/pt/docs/teamverwaltung/das-komplette-team-loeschen/).
